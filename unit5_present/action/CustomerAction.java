package by.epam.unit5.action;

import by.epam.unit5.builder.CandyPresentBuilder;
import by.epam.unit5.builder.PresentDirector;
import by.epam.unit5.entity.*;
...
public class CustomerAction {
    ...
    public void run() {
        ...
        System.out.println(customer.getName() + ", who do you want to give a present to?");
        customer.setPresent(presentDirector.buildPresent());
        customer.getPresent().setProducts(fillProducts());

        System.out.println("Your present for " + customer.getPresent().getName() + " is ready!");
    ...
    private static List<Product> readCandyFile() {
        ...
        String filePath = "src/by/unit5/present/data/candies.txt";// здесь проверь
        try {
